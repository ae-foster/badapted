...
class BayesianAdaptiveDesignGenerator(DesignGeneratorABC):
    ...
    def get_next_design(self, model):
        ...
        chosen_design_df, _ = design_optimisation(
            allowable_designs,
            model.predictive_y,
            model.θ,
            n_steps=50,
    ...
    def _refine_design_space(self, model, allowable_designs):
        ...
        if not self.allow_repeats and self.trial > 1:
            ...
            allowable_designs = allowable_designs.loc[
                ~allowable_designs.isin(designs_to_exclude.to_dict(orient="list")).all(
                    axis=1
                ),
                :,
            ]
    ...
    def _convert_to_ranks(self, input_designs):
        ...
        for n, design_variable in enumerate(design_variable_names):
            ...
            all_designs_this_variable = self._get_sorted_unique_design_vals(
                design_variable
            )
            ...
            if N_this == 1:
                ...
                # (0:(N_this-1))'/(N_this-1))
                design_ranks[:, n] = np.interp(
                    input_designs[:, n],
                    all_designs_this_variable,
                    np.linspace(0.0, 1.0, num=N_this),
                )
    ...
    def _default_penalty_func(self, candidate_designs, base_sigma=1, λ=2):
        ...
        candidate_designs = np.concatenate(
            (candidate_designs, previous_designs), axis=0
        )
        ...
        for n in range(n_previous_designs):
            densities[:, n] = multivariate_normal.pdf(
                candidate_designs, mean=previous_designs[n, :], cov=sigma
            )
...
def _remove_highly_predictable_designs(allowable_designs, model):
    ...
    while n_not_predictable > 200 and threshold < max_threshold:
        threshold *= 1.05
        highly_predictable, n_predictable, n_not_predictable = _calc_predictability(
            allowable_designs, threshold
        )
